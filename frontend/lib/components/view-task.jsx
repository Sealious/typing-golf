...
var ViewTask = React.createClass({
	...
	fetch: function() {
		...
		setTimeout(function() {
			qwest
				.get("/api/v1/resources/task/" + self.props.params.id)
	...
	componentDidUpdate: function(prevProps, prevState) {
		try {
			if (
				equal(this.state.current_state, this.state.to) &&
				this.state.loaded
			) {
	...
	handleChange: function(new_data) {
		...
		if (
			new_data.text !== this.state.current_state.text ||
	...
	postScore: function(nick) {
		var self = this;
		qwest
			.post("/api/v1/resources/ranking_entry", {
				nick: nick,
